...
This example displays all 15 marching cubes cases (0-14) in a viewport grid.

**UnstructuredGrid → ContourFilter → ExtractEdges → TubeFilter → Mapper → Actor | ShrinkPolyData → Mapper → Actor | ThresholdPoints → Glyph3D → Mapper → Actor → Renderer (×15) → Window → Interactor**
