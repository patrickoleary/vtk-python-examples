...
# Towers of Hanoi: display an intermediate configuration captured mid-solve.
...
render_window.SetWindowName("HanoiIntermediate")
...
for i in range(num_pucks):
    ...
    peg_stack[0].append(puck)
    pucks.append(puck)

render_window.Render()

# Animate partway to capture an intermediate state
num_moves = 0
...
def move_puck(peg1, peg2):
    global num_moves, got_intermediate
    num_moves += 1
    ...
    for step in range(num_steps):
        ...
        render_window.Render()
        if num_moves == CAPTURE_MOVE and step == CAPTURE_STEP:
            intermediate_cam = vtkCamera()
            intermediate_cam.SetPosition(54.7263, 41.6467, 44.125)
            intermediate_cam.SetFocalPoint(11.5603, -1.51931, 0.95899)
            intermediate_cam.SetClippingRange(42.4226, 115.659)
            intermediate_cam.SetViewUp(0, 1, 0)
            renderer.SetActiveCamera(intermediate_cam)
            render_window.Render()
...
# Animate until the intermediate configuration is captured
hanoi(num_pucks - 1, 0, 2, 1)
if not got_intermediate:
    move_puck(0, 1)
if not got_intermediate:
    hanoi(num_pucks - 1, 2, 1, 0)
