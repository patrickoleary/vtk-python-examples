...
This example illustrates camera movement around the focal point (azimuth, elevation, roll).

**Sources → ImplicitModel → Contour → Warp → Transform → Mapper → Actors → Text → Renderer → Window → Interactor**

- [vtkAppendFilter](https://www.vtk.org/doc/nightly/html/classvtkAppendFilter.html) combines the camera lens and body.
...
- [vtkTextActor](https://www.vtk.org/doc/nightly/html/classvtkTextActor.html) displays text labels for azimuth, elevation, and roll.
