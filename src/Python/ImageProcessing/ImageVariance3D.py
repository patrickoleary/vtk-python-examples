...
# Compute local variance of a medical volume slice using
# vtkImageVariance3D and display original vs variance side by side.
...
# Variance: compute local variance in a 5x5x1 neighborhood
variance = vtkImageVariance3D()
variance.SetInputConnection(reader.GetOutputPort())
variance.SetKernelSize(5, 5, 1)

# Cast variance to unsigned char for display
cast_variance = vtkImageCast()
cast_variance.SetInputConnection(variance.GetOutputPort())
cast_variance.SetOutputScalarTypeToUnsignedChar()
cast_variance.ClampOverflowOn()
...
# Actor 2: variance slice (right viewport)
actor_variance = vtkImageActor()
actor_variance.GetMapper().SetInputConnection(cast_variance.GetOutputPort())
actor_variance.SetDisplayExtent(extent[0], extent[1], extent[2], extent[3], mid_z, mid_z)
...
# Renderer 2: right viewport — variance
renderer_right = vtkRenderer()
renderer_right.SetViewport(0.5, 0.0, 1.0, 1.0)
renderer_right.AddActor(actor_variance)
...
render_window.SetWindowName("ImageVariance3D")
