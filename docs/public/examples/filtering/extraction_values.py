...
# Demonstrate vtkExtractSelection with VALUES selection type
...
# --- VALUES on cells: preserve topology ---
...
# --- VALUES on points: preserve topology ---
selection_node.GetProperties().Set(vtkSelectionNode.INVERSE(), 0)
...
# --- VALUES on points: preserve topology + containing cells ---
selection_node.GetProperties().Set(vtkSelectionNode.CONTAINING_CELLS(), 1)
extract_selection.Update()
...
cell_select_copy.GetCellData().SetActiveScalars(cell_reverse_ids.GetName())
cell_select_mapper.SetScalarModeToUseCellData()
cell_select_mapper.SetScalarRange(cell_reverse_ids.GetRange())

# Mapper — cell inverse
cell_inverse_mapper = vtkDataSetMapper()
cell_inverse_mapper.SetInputData(cell_inverse_copy)
cell_inverse_copy.GetCellData().SetActiveScalars(cell_reverse_ids.GetName())
cell_inverse_mapper.SetScalarModeToUseCellData()
cell_inverse_mapper.SetScalarRange(cell_reverse_ids.GetRange())
...
point_select_copy.GetPointData().SetActiveScalars(point_reverse_ids.GetName())
point_select_mapper.SetScalarModeToUsePointData()
point_select_mapper.SetScalarRange(point_reverse_ids.GetRange())

# Mapper — point inverse
point_inverse_mapper = vtkDataSetMapper()
point_inverse_mapper.SetInputData(point_inverse_copy)
point_inverse_copy.GetPointData().SetActiveScalars(point_reverse_ids.GetName())
point_inverse_mapper.SetScalarModeToUsePointData()
point_inverse_mapper.SetScalarRange(point_reverse_ids.GetRange())
...
point_containing_copy.GetPointData().SetActiveScalars(point_reverse_ids.GetName())
point_containing_mapper.SetScalarModeToUsePointData()
point_containing_mapper.SetScalarRange(point_reverse_ids.GetRange())
...
render_window.SetWindowName("extraction values")
