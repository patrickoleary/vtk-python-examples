...
flat_polydata = vtkPolyData()
flat_polydata.SetPoints(flat_points)

# Filter: Delaunay triangulation
delaunay = vtkDelaunay2D()
delaunay.SetInputData(flat_polydata)

# Filter: compute cell quality (aspect ratio)
quality_filter = vtkCellQuality()
quality_filter.SetInputConnection(delaunay.GetOutputPort())
quality_filter.SetQualityMeasureToAspectRatio()
quality_filter.Update()

# Mapper: map the mesh to graphics primitives with quality coloring
mapper = vtkPolyDataMapper()
mapper.SetInputConnection(quality_filter.GetOutputPort())
mapper.SetScalarModeToUseCellData()
mapper.SelectColorArray("CellQuality")
mapper.SetScalarRange(quality_filter.GetOutput().GetCellData().GetArray("CellQuality").GetRange())
...
render_window.SetWindowName("cell quality")
