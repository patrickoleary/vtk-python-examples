...
# Test vtkImageBSplineCoefficients displaying original vs coefficient image.
...
data_range = list(reader.GetOutput().GetScalarRange())

# B-spline coefficients
bspline_coefficients = vtkImageBSplineCoefficients()
bspline_coefficients.SetInputConnection(reader.GetOutputPort())
bspline_coefficients.SetSplineDegree(3)
bspline_coefficients.Update()

# Viewport 0: original image
...
# Viewport 1: B-spline coefficients
image_mapper_1 = vtkImageSliceMapper()
image_mapper_1.SetInputConnection(bspline_coefficients.GetOutputPort())

image_slice_1 = vtkImageSlice()
image_slice_1.SetMapper(image_mapper_1)
image_slice_1.GetProperty().SetColorWindow(data_range[1] - data_range[0])
image_slice_1.GetProperty().SetColorLevel(0.5 * (data_range[0] + data_range[1]))
...
render_window.SetWindowName("image b spline coefficients")
