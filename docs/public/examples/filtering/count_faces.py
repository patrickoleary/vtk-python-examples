...
# Demonstrate vtkCountFaces by building an unstructured grid with various
# cell types (vertex, line, triangle, tetra, pyramid, wedge, voxel,
# hexahedron, pentagonal prism, hexagonal prism) and visualizing the
# cells colored by face count.
...
from vtkmodules.vtkFiltersGeneral import vtkCountFaces
...
# Pentagonal prism (10 points) at x=23
for i in range(5):
...
# Hexagonal prism (12 points) at x=26
for i in range(6):
...
# Count faces
count_faces = vtkCountFaces()
count_faces.SetOutputArrayName("faces")
count_faces.SetInputData(ugrid)
count_faces.Update()

# Extract surface and color by face count
surface = vtkDataSetSurfaceFilter()
surface.SetInputConnection(count_faces.GetOutputPort())

mapper = vtkPolyDataMapper()
mapper.SetInputConnection(surface.GetOutputPort())
mapper.SetScalarModeToUseCellFieldData()
mapper.SelectColorArray("faces")
...
render_window.SetWindowName("count faces")
