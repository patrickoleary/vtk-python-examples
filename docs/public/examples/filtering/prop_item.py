#!/usr/bin/env python
# Demonstrate vtkPropItem with DEM data, labeled contours, and fixed aspect context area.
...
contour_mapper = vtkLabeledContourMapper()
...
contour_mapper.SetTextProperty(tprop)
...
contour_mapper.GetPolyDataMapper().SetLookupTable(contour_lut)
contour_mapper.GetPolyDataMapper().SetScalarRange(scalar_range)
...
# Set up the context area with fixed aspect and grid off.
area = vtkContextArea()
area.ShowGridOff()
...
render_window.SetWindowName("prop item")
