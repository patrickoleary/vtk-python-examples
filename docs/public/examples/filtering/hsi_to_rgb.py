...
# Round-trip RGB to HSI and back to RGB conversion on a canvas image.
...
# RGB -> HSI -> RGB round trip
convert = vtkImageRGBToHSI()
convert.SetInputConnection(image_canvas.GetOutputPort())

convert_back = vtkImageHSIToRGB()
convert_back.SetInputConnection(convert.GetOutputPort())

cast = vtkImageCast()
cast.SetInputConnection(convert_back.GetOutputPort())
...
image_actor.GetMapper().SetInputConnection(convert_back.GetOutputPort())
...
render_window.SetWindowName("hsi to rgb")
