...
# Test vtkImageReslice with reslice axes vs reslice transform, input sampling on/off.
...
# Viewport 0 (lower-left): axes, input sampling off
reslice_0 = vtkImageReslice()
reslice_0.SetInputConnection(reader.GetOutputPort())
reslice_0.SetOutputSpacing(1.0, 1.0, 1.0)
reslice_0.TransformInputSamplingOff()
reslice_0.SetResliceAxes(transform.GetMatrix())
...
# Viewport 1 (lower-right): axes, input sampling on
reslice_1 = vtkImageReslice()
reslice_1.SetInputConnection(reader.GetOutputPort())
reslice_1.SetOutputSpacing(1.0, 1.0, 1.0)
reslice_1.TransformInputSamplingOn()
reslice_1.SetResliceAxes(transform.GetMatrix())
...
# Viewport 2 (upper-left): transform, input sampling off
reslice_2 = vtkImageReslice()
reslice_2.SetInputConnection(reader.GetOutputPort())
reslice_2.SetOutputSpacing(1.0, 1.0, 1.0)
reslice_2.TransformInputSamplingOff()
reslice_2.SetResliceTransform(transform)
...
# Viewport 3 (upper-right): transform, input sampling on
reslice_3 = vtkImageReslice()
reslice_3.SetInputConnection(reader.GetOutputPort())
reslice_3.SetOutputSpacing(1.0, 1.0, 1.0)
reslice_3.TransformInputSamplingOn()
reslice_3.SetResliceTransform(transform)
...
render_window.SetWindowName("image reslice imaging")
