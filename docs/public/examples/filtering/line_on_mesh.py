...
clean_filter.SetInputData(triangle_polydata)
...
subdivision_filter.SetInputConnection(clean_filter.GetOutputPort())
subdivision_filter.Update()

# Mapper: map the smoothed mesh to graphics primitives
terrain_mapper = vtkPolyDataMapper()
terrain_mapper.SetInputConnection(subdivision_filter.GetOutputPort())
...
cell_locator.SetDataSet(subdivision_filter.GetOutput())
cell_locator.BuildLocator()

max_samples = 1000
sample_spacing = 20.0 / max_samples
intersection_tolerance = 0.001

# Intersection: cast vertical lines and collect surface intersection points
intersection_points = vtkPoints()
for i in range(max_samples):
    p1 = [2 + i * sample_spacing, 16, -1]
    p2 = [2 + i * sample_spacing, 16, 6]

    parametric_t = mutable(0)
    intersection_position = [0.0, 0.0, 0.0]
    parametric_coords = [0.0, 0.0, 0.0]
    sub_cell_id = mutable(0)
    cell_locator.IntersectWithLine(p1, p2, intersection_tolerance, parametric_t,
                                   intersection_position, parametric_coords, sub_cell_id)

    intersection_position[2] += 0.01
    intersection_points.InsertNextPoint(intersection_position)

# Spline: fit a parametric spline through the intersection points
parametric_spline = vtkParametricSpline()
parametric_spline.SetPoints(intersection_points)
spline_source = vtkParametricFunctionSource()
spline_source.SetUResolution(max_samples)
...
render_window.SetWindowName("line on mesh")
