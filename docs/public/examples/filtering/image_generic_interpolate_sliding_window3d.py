...
# Test vtkGenericImageInterpolator SlidingWindow on 3D volume with four views.
...
# Generic interpolator with nearest-neighbor and sliding window
nearest = vtkGenericImageInterpolator()
nearest.SetInterpolationModeToNearest()
nearest.SlidingWindowOn()

reslice = vtkImageReslice()
reslice.SetInputConnection(reader.GetOutputPort())
reslice.SetOutputSpacing(0.80, 0.80, 1.5)
reslice.SetOutputScalarType(VTK_FLOAT)
reslice.SetInterpolator(nearest)
reslice.Update()

color_range = [0, 4095]

# Viewport 0 (lower-left): orientation 0
...
# Viewport 1 (lower-right): orientation 1
...
# Viewport 2 (upper-left): orientation 2
...
# Viewport 3 (upper-right): orientation 0
image_mapper_3 = vtkImageSliceMapper()
image_mapper_3.SetInputConnection(reslice.GetOutputPort())
...
render_window.SetWindowName("image generic interpolate sliding window3d")
