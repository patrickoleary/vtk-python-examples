...
# Demonstrate vtkExtractSelection with FRUSTUM selection type
...
cell_frust_corners = vtkDoubleArray()
cell_frust_corners.SetNumberOfComponents(4)
cell_frust_corners.SetNumberOfTuples(8)
cell_frust_corners.SetTuple4(0, 0.1, 0.1, 3.1, 0.0)
cell_frust_corners.SetTuple4(1, 0.1, 0.1, 0.1, 0.0)
cell_frust_corners.SetTuple4(2, 0.1, 0.9, 3.1, 0.0)
cell_frust_corners.SetTuple4(3, 0.1, 0.9, 0.1, 0.0)
cell_frust_corners.SetTuple4(4, 0.9, 0.1, 3.1, 0.0)
cell_frust_corners.SetTuple4(5, 0.9, 0.1, 0.1, 0.0)
cell_frust_corners.SetTuple4(6, 0.9, 0.9, 3.1, 0.0)
cell_frust_corners.SetTuple4(7, 0.9, 0.9, 0.1, 0.0)
selection_node.SetSelectionList(cell_frust_corners)

extract_selection.Update()
cell_select_copy = extract_selection.GetOutput().NewInstance()
cell_select_copy.DeepCopy(extract_selection.GetOutput())

# --- FRUSTUM on cells: inverse ---
...
# --- FRUSTUM on cells: preserve topology ---
...
# --- FRUSTUM on points: select ---
selection_node.Initialize()
extract_selection.PreserveTopologyOff()
selection_node.GetProperties().Set(vtkSelectionNode.CONTENT_TYPE(), vtkSelectionNode.FRUSTUM)
selection_node.GetProperties().Set(vtkSelectionNode.FIELD_TYPE(), vtkSelectionNode.POINT)
point_frust_corners = vtkDoubleArray()
point_frust_corners.SetNumberOfComponents(4)
point_frust_corners.SetNumberOfTuples(8)
point_frust_corners.SetTuple4(0, -0.1, -0.1, 3.1, 0.0)
point_frust_corners.SetTuple4(1, -0.1, -0.1, -0.1, 0.0)
point_frust_corners.SetTuple4(2, -0.1, 0.1, 3.1, 0.0)
point_frust_corners.SetTuple4(3, -0.1, 0.1, -0.1, 0.0)
point_frust_corners.SetTuple4(4, 0.1, -0.1, 3.1, 0.0)
point_frust_corners.SetTuple4(5, 0.1, -0.1, -0.1, 0.0)
point_frust_corners.SetTuple4(6, 0.1, 0.1, 3.1, 0.0)
point_frust_corners.SetTuple4(7, 0.1, 0.1, -0.1, 0.0)
selection_node.SetSelectionList(point_frust_corners)

extract_selection.Update()
point_select_copy = extract_selection.GetOutput().NewInstance()
point_select_copy.DeepCopy(extract_selection.GetOutput())

# --- FRUSTUM on points: inverse ---
selection_node.GetProperties().Set(vtkSelectionNode.INVERSE(), 1)
extract_selection.Update()
point_inverse_copy = extract_selection.GetOutput().NewInstance()
point_inverse_copy.DeepCopy(extract_selection.GetOutput())

# --- FRUSTUM on points: containing cells ---
selection_node.GetProperties().Set(vtkSelectionNode.INVERSE(), 0)
...
# --- FRUSTUM on points: preserve topology ---
selection_node.GetProperties().Set(vtkSelectionNode.INVERSE(), 0)
...
# --- FRUSTUM on points: preserve topology + containing cells ---
selection_node.GetProperties().Set(vtkSelectionNode.CONTAINING_CELLS(), 1)
extract_selection.Update()
...
cell_select_copy.GetCellData().SetActiveScalars(cell_counter_array.GetName())
cell_select_mapper.SetScalarModeToUseCellData()
cell_select_mapper.SetScalarRange(cell_counter_array.GetRange())

# Mapper — cell inverse
cell_inverse_mapper = vtkDataSetMapper()
cell_inverse_mapper.SetInputData(cell_inverse_copy)
cell_inverse_copy.GetCellData().SetActiveScalars(cell_counter_array.GetName())
cell_inverse_mapper.SetScalarModeToUseCellData()
cell_inverse_mapper.SetScalarRange(cell_counter_array.GetRange())
...
point_select_copy.GetPointData().SetActiveScalars(point_counter_array.GetName())
point_select_mapper.SetScalarModeToUsePointData()
point_select_mapper.SetScalarRange(point_counter_array.GetRange())

# Mapper — point inverse
point_inverse_mapper = vtkDataSetMapper()
point_inverse_mapper.SetInputData(point_inverse_copy)
point_inverse_copy.GetPointData().SetActiveScalars(point_counter_array.GetName())
point_inverse_mapper.SetScalarModeToUsePointData()
point_inverse_mapper.SetScalarRange(point_counter_array.GetRange())
...
point_containing_copy.GetPointData().SetActiveScalars(point_counter_array.GetName())
point_containing_mapper.SetScalarModeToUsePointData()
point_containing_mapper.SetScalarRange(point_counter_array.GetRange())
...
render_window.SetWindowName("extraction frustum")
