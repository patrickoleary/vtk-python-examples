#!/usr/bin/env python
# Demonstrate a scatter plot matrix with histogram bins, repopulated with different point counts.
...
render_window.SetWindowName("scatter plot matrix histogram")
...
# Repopulate with more points.
table_2 = vtkTable()
arr_x_2 = vtkFloatArray()
arr_x_2.SetName("x")
table_2.AddColumn(arr_x_2)
arr_c_2 = vtkFloatArray()
arr_c_2.SetName("cos(x)")
table_2.AddColumn(arr_c_2)
arr_s_2 = vtkFloatArray()
arr_s_2.SetName("sin(x)")
table_2.AddColumn(arr_s_2)
arr_s2_2 = vtkFloatArray()
arr_s2_2.SetName("sin(x + 0.5)")
table_2.AddColumn(arr_s2_2)
tangent_2 = vtkFloatArray()
tangent_2.SetName("tan(x)")
table_2.AddColumn(tangent_2)

num_points_2 = 400
inc_2 = 4.0 * math.pi / (num_points_2 - 1)
table_2.SetNumberOfRows(num_points_2)
for i in range(num_points_2):
    table_2.SetValue(i, 0, i * inc_2)
    table_2.SetValue(i, 1, math.cos(i * inc_2))
    table_2.SetValue(i, 2, math.sin(i * inc_2))
    table_2.SetValue(i, 3, math.sin(i * inc_2) + 0.5)
    table_2.SetValue(i, 4, math.tan(i * inc_2))

matrix.SetInput(table_2)
matrix.SetNumberOfBins(7)
