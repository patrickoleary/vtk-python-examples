...
# Test vtkImageHistogram displaying original image alongside histogram image.
...
bin_range = [
    histogram.GetBinOrigin(),
    histogram.GetBinOrigin() + (num_bins - 1) * histogram.GetBinSpacing(),
]
...
image_slice_0.GetProperty().SetColorWindow(bin_range[1] - bin_range[0])
image_slice_0.GetProperty().SetColorLevel(0.5 * (bin_range[0] + bin_range[1]))
...
# Viewport 1: histogram image
image_mapper_1 = vtkImageSliceMapper()
image_mapper_1.SetInputConnection(histogram.GetOutputPort())
...
image_slice_1.GetProperty().SetColorWindow(255.0)
image_slice_1.GetProperty().SetColorLevel(127.5)
...
render_window.SetWindowName("image histogram imaging")
