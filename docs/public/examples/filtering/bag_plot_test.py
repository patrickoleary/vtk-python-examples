#!/usr/bin/env python
# Demonstrate a bag plot with density data using vtkPlotBag.

import vtkmodules.vtkInteractionStyle  # noqa: F401
import vtkmodules.vtkRenderingContextOpenGL2  # noqa: F401
import vtkmodules.vtkRenderingOpenGL2  # noqa: F401

from vtkmodules.vtkChartsCore import vtkChartXY, vtkPlotBag
...
arr_y = vtkDoubleArray()
arr_y.SetName("Y")

arr_density = vtkDoubleArray()
...
table.SetNumberOfRows(num_data_i * num_data_j)

for j in range(num_data_j):
    for i in range(num_data_i):
        idx = i + j * num_data_i
        table.SetValue(idx, 0, i)
        table.SetValue(idx, 1, j)
        table.SetValue(idx, 2, densities[idx])
...
bag_plot = vtkPlotBag()
chart.AddPlot(bag_plot)
bag_plot.SetInputData(table, "X", "Y", "Density")
bag_plot.SetColor(255, 0, 0, 255)
bag_plot.SetMarkerSize(4)
...
render_window.SetWindowName("bag plot test")
