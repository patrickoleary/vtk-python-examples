...
# Cut an overlapping AMR dataset with an oblique plane using
# vtkAMRCutPlane and render the cut surface colored by a Gaussian
# pulse scalar field alongside block outlines.
...
from vtkmodules.vtkFiltersAMR import vtkAMRCutPlane
...
# CutPlane: cut the AMR dataset with an oblique plane through the center
cutter = vtkAMRCutPlane()
cutter.SetInputData(amr)
cutter.SetNormal(1.0, 1.0, 0.0)
cutter.SetCenter(5.0, 5.0, 5.0)
cutter.SetLevelOfResolution(1)

cut_geom = vtkCompositeDataGeometryFilter()
cut_geom.SetInputConnection(cutter.GetOutputPort())

cut_mapper = vtkPolyDataMapper()
cut_mapper.SetInputConnection(cut_geom.GetOutputPort())
cut_mapper.SetScalarRange(0.0, 1.0)
...
render_window.SetWindowName("amr cut plane")
