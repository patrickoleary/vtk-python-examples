...
# Slice a hyper tree grid with a plane using vtkHyperTreeGridPlaneCutter
# and render the cut surface alongside a translucent wireframe of the
# full HTG for context.
...
# Filter: cut the HTG with an oblique plane through the center
plane_cutter = vtkHyperTreeGridPlaneCutter()
plane_cutter.SetInputConnection(source.GetOutputPort())
plane_cutter.SetPlane(1.0, 0.0, -1.0, 2.5)

# Mapper: map the cut surface to graphics primitives, colored by Depth
cut_mapper = vtkPolyDataMapper()
cut_mapper.SetInputConnection(plane_cutter.GetOutputPort())
cut_mapper.SetScalarModeToUseCellFieldData()
cut_mapper.SelectColorArray("Depth")
cut_mapper.SetScalarRange(0.0, 4.0)

# Actor: assign the cut surface
cut_actor = vtkActor()
cut_actor.SetMapper(cut_mapper)
cut_actor.GetProperty().EdgeVisibilityOn()
cut_actor.GetProperty().SetEdgeColor(0.0, 0.0, 0.0)
...
render_window.SetWindowName("hypertreegrid plane cutter")
