...
# field and render it with block outlines and an iso-surface colored by
# the pulse value.
...
# Contour: iso-surface at the half-maximum of the Gaussian pulse
contour = vtkContourFilter()
contour.SetInputData(amr)
contour.SetNumberOfContours(1)
contour.SetValue(0, 0.5)

contour_geom = vtkCompositeDataGeometryFilter()
contour_geom.SetInputConnection(contour.GetOutputPort())

contour_mapper = vtkPolyDataMapper()
contour_mapper.SetInputConnection(contour_geom.GetOutputPort())
...
render_window.SetWindowName("amr gaussian pulse")
