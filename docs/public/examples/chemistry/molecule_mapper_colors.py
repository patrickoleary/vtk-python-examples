#!/usr/bin/env python
# Demonstrate vtkMoleculeMapper with custom RGB color array and MapScalars off.

import vtkmodules.vtkInteractionStyle  # noqa: F401
import vtkmodules.vtkRenderingOpenGL2  # noqa: F401

from vtkmodules.vtkCommonCore import vtkDoubleArray
from vtkmodules.vtkCommonDataModel import vtkDataObject, vtkMolecule
...
# Build RGB color array (cycling R, G, B per atom).
colors = vtkDoubleArray()
colors.SetName("Colors")
colors.SetNumberOfComponents(3)
colors.Allocate(3 * mol.GetNumberOfAtoms())
for i in range(mol.GetNumberOfAtoms()):
    c = [0.0, 0.0, 0.0]
    c[i % 3] = 1.0
    colors.InsertNextTypedTuple(c)
mol.GetAtomData().AddArray(colors)

# Molecule mapper with custom color array.
mol_mapper = vtkMoleculeMapper()
mol_mapper.SetInputData(mol)
mol_mapper.SetInputArrayToProcess(
...
render_window.SetWindowName("molecule mapper colors")
