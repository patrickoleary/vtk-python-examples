#!/usr/bin/env python
# Demonstrate composite molecule rendering with opaque ball-and-stick and translucent VDW spheres.
...
# Opaque ball-and-stick mapper.
bs_mapper = vtkMoleculeMapper()
bs_mapper.SetInputData(mol)
bs_mapper.UseBallAndStickSettings()

bs_actor = vtkActor()
bs_actor.SetMapper(bs_mapper)

# Translucent VDW spheres mapper (no bonds).
vdw_mapper = vtkMoleculeMapper()
vdw_mapper.SetInputData(mol)
vdw_mapper.UseVDWSpheresSettings()
vdw_mapper.RenderBondsOff()

vdw_actor = vtkActor()
vdw_actor.SetMapper(vdw_mapper)
vdw_actor.GetProperty().SetOpacity(0.15)

# Rendering pipeline.
renderer = vtkRenderer()
renderer.AddActor(bs_actor)
renderer.AddActor(vdw_actor)
...
render_window.SetWindowName("composite render")
