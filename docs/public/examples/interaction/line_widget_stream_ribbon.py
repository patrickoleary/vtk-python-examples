#!/usr/bin/env python
# Demonstrate vtkLineWidget2 seeding streamlines on PLOT3D data with ribbon visualization.
...
render_window.SetWindowName("line widget stream ribbon")
...
def line_callback(caller, event_string):
    ...
    widget_rep.GetPolyData(seeds)
    streamline_actor.VisibilityOn()


# Widget
line_rep = vtkLineRepresentation()
line_rep.SetPoint1WorldPosition([0.0, -1.0, 0.0])
line_rep.SetPoint2WorldPosition([0.0, 1.0, 0.0])
line_rep.PlaceWidget(plot3d_block0.GetBounds())
line_rep.GetPolyData(seeds)
line_rep.DistanceAnnotationVisibilityOn()

line_widget = vtkLineWidget2()
line_widget.SetInteractor(interactor)
line_widget.SetRepresentation(line_rep)
