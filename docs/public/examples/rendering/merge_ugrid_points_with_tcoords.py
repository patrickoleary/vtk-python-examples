...
# Demonstrate vtkStaticCleanUnstructuredGrid merging unstructured grid
# points with texture coordinates, verifying that points are only
# merged when both geometry and TCoords match.
...
# First plane patch (unstructured grid, upper-left)
unstructured_grid_0 = vtkUnstructuredGrid()
points_0 = vtkPoints()
unstructured_grid_0.SetPoints(points_0)
...
unstructured_grid_0.GetPointData().SetTCoords(t_0)

unstructured_grid_0.Allocate(20)
for cell in [[0, 1, 4, 3], [1, 2, 5, 4], [3, 4, 7, 6], [4, 5, 8, 7]]:
    unstructured_grid_0.InsertNextCell(9, 4, cell)

# Second plane patch (polydata, upper-right)
...
polydata_1.Allocate(20)
for cell in [[0, 1, 4, 3], [1, 2, 5, 4], [3, 4, 7, 6], [4, 5, 8, 7]]:
    polydata_1.InsertNextCell(9, 4, cell)

# Third plane patch (unstructured grid, lower-left)
unstructured_grid_2 = vtkUnstructuredGrid()
points_2 = vtkPoints()
unstructured_grid_2.SetPoints(points_2)
...
unstructured_grid_2.GetPointData().SetTCoords(t_2)

unstructured_grid_2.Allocate(20)
for cell in [[0, 1, 4, 3], [1, 2, 5, 4], [3, 4, 7, 6], [4, 5, 8, 7],
             [6, 7, 10, 9], [7, 8, 11, 10], [9, 10, 13, 12], [10, 11, 14, 13]]:
    unstructured_grid_2.InsertNextCell(9, 4, cell)

# Fourth plane patch (unstructured grid, lower-right)
unstructured_grid_3 = vtkUnstructuredGrid()
points_3 = vtkPoints()
unstructured_grid_3.SetPoints(points_3)
...
unstructured_grid_3.GetPointData().SetTCoords(t_3)

unstructured_grid_3.Allocate(20)
for cell in [[0, 1, 4, 3], [1, 2, 5, 4], [3, 4, 7, 6], [4, 5, 8, 7],
             [6, 7, 10, 9], [7, 8, 11, 10], [9, 10, 13, 12], [10, 11, 14, 13]]:
    unstructured_grid_3.InsertNextCell(9, 4, cell)

# Append all datasets
append = vtkAppendDataSets()
append.AddInputData(unstructured_grid_0)
append.AddInputData(polydata_1)
append.AddInputData(unstructured_grid_2)
append.AddInputData(unstructured_grid_3)
append.Update()
...
render_window.SetWindowName("merge ugrid points with tcoords")
