...
# Demonstrate multi-texturing with interpolated scalars and a discretized color transfer function.
...
pd = elevation.GetOutput()
tcoord = pd.GetPointData().GetTCoords()
tcoord.SetName("TexCoords")
...
# Cloud texture with modulate blending
reader = vtkJPEGReader()
reader.SetFileName(os.path.join(data_dir, "clouds.jpeg"))
tex = vtkTexture()
tex.InterpolateOn()
tex.SetBlendingMode(1)  # VTK_TEXTURE_BLENDING_MODE_MODULATE
...
mapper.MapDataArrayToMultiTextureAttribute(
    "skyclouds", "TexCoords", vtkDataObject.FIELD_ASSOCIATION_POINTS
)
...
render_window.SetWindowName("multi texturing interpolate scalars")
