...
# Demonstrate PBR clear coat rendering on a cube with coat normal texture.
...
# Coat normal texture (flipped)
flip_normal = vtkImageFlip()
flip_normal.SetInputConnection(normal_reader.GetOutputPort())
flip_normal.SetFilteredAxes(0)
coat_normal = vtkTexture()
coat_normal.InterpolateOn()
coat_normal.SetInputConnection(flip_normal.GetOutputPort())

# PBR actor with clear coat
actor = vtkActor()
...
actor.GetProperty().SetCoatColor(1.0, 0.0, 0.0)
...
render_window.SetWindowName("pbr clear coat opengl")
