...
class SliceInteractorStyle(vtkInteractorStyleImage):
    def __init__(self, image_actor, status_text_actor, min_slice, max_slice,
                 renderer, render_window):
        super().__init__()
        self.image_actor = image_actor
        self.status_text_actor = status_text_actor
    ...
    def _move(self, delta):
        ...
        self.status_text_actor.SetInput(
...
slice_text_actor = vtkTextActor()
slice_text_actor.SetInput(f"Slice 1/{max_slice - min_slice + 1}")
slice_text_actor.SetPosition(15, 10)
slice_text_actor.GetTextProperty().ShallowCopy(slice_text_prop)
...
usage_text_actor = vtkTextActor()
usage_text_actor.SetInput(
...
usage_text_actor.GetTextProperty().ShallowCopy(usage_text_prop)
...
render_window.SetWindowName("read dicom series app")
...
slice_style = SliceInteractorStyle(
    actor, slice_text_actor, min_slice, max_slice, renderer, render_window)
