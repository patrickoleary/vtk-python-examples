...
# Read a PLY file with per-face texture UV coordinates and render with a PNG texture.
...
ply_reader.SetFileName(os.path.join(data_dir, "squareTexturedFaces.ply"))
ply_reader.Update()

png_reader = vtkPNGReader()
png_reader.SetFileName(os.path.join(data_dir, "two_vtk_logos_stacked.png"))
png_reader.Update()
...
render_window.SetWindowName("ply reader texture uv faces")
