...
# Demonstrate appending data to vtkImplicitModeller without explicit bounds,
# using ComputeModelBounds instead.
...
# Create a cube to define bounds
cube_for_bounds = vtkCubeSource()
cube_for_bounds.SetBounds(-2.5, 2.5, -2.5, 2.5, -2.5, 2.5)
cube_for_bounds.Update()
...
# Build implicit model by appending (bounds computed from cube)
imp = vtkImplicitModeller()
imp.SetSampleDimensions(60, 60, 60)
imp.SetCapValue(1000)
imp.ComputeModelBounds(cube_for_bounds.GetOutput())
...
render_window.SetWindowName("append implicit model no bounds")
