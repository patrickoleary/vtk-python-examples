...
# Test vtkCompositeSurfaceLICMapper: SurfaceLICMultiBlockContrastEnhancedPerlin.
...
reader.SetFileName(os.path.join(data_dir, "ex-blow_5.vtm"))
...
while not it.IsDoneWithTraversal():
    ...
    if ds and ds.GetNumberOfCells() > 0:
        v = ds.GetPointData().GetArray("DISPLACEMENT_")
        if v is not None:
            mag = vtkFloatArray()
            mag.SetName("magDISPLACEMENT_")
...
lic_mapper.SetInputArrayToProcess(
    0, 0, 0, vtkDataObject.FIELD_ASSOCIATION_POINTS_THEN_CELLS, "DISPLACEMENT_")
...
lic_interface.SetNumberOfSteps(50)
lic_interface.SetStepSize(0.1)
lic_interface.SetEnhancedLIC(1)
lic_interface.SetNormalizeVectors(1)
lic_interface.SetGenerateNoiseTexture(1)
lic_interface.SetNoiseType(2)
lic_interface.SetNoiseTextureSize(128)
...
lic_interface.SetEnhanceContrast(4)
...
lic_interface.SetMapModeBias(0.1)
lic_interface.SetMaskOnSurface(1)
lic_interface.SetMaskThreshold(0.0)
lic_interface.SetMaskIntensity(0.41)
lic_interface.SetMaskColor([0.0, 0.0, 0.0])
...
render_window.SetSize(364, 256)
render_window.AddRenderer(renderer)
render_window.SetWindowName("surface lic multiblock contrast enhanced perlin")
