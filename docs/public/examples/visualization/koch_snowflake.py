...
subdivide_stack = [
    (indices[0 : stride + 1], 1),
    (indices[stride : 2 * stride + 1], 1),
    (indices[2 * stride : -1], 1),
]
while subdivide_stack:
    idx, level = subdivide_stack.pop()
    if len(idx) >= 3:
        s = len(idx) // 4
        idx.append(idx[-1] + 1)
        tri = vtkTriangle()
        ...
        subdivide_stack.append((idx[0:s], level + 1))
        subdivide_stack.append((idx[s : 2 * s], level + 1))
        subdivide_stack.append((idx[2 * s : 3 * s], level + 1))
        subdivide_stack.append((idx[3 * s : -1], level + 1))
...
render_window.SetWindowName("koch snowflake")
