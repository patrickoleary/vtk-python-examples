#!/usr/bin/env python
# Demonstrate vtkKochanekSpline interpolation through random 3D points.
...
spline_x = vtkKochanekSpline()
spline_y = vtkKochanekSpline()
spline_z = vtkKochanekSpline()
...
glyph_actor.GetProperty().SetDiffuseColor(1, 0.6, 0.6)
...
bias = 0
continuity = 0
spline_x.SetDefaultTension(tension)
spline_x.SetDefaultBias(bias)
spline_x.SetDefaultContinuity(continuity)
spline_y.SetDefaultTension(tension)
spline_y.SetDefaultBias(bias)
spline_y.SetDefaultContinuity(continuity)
spline_z.SetDefaultTension(tension)
spline_z.SetDefaultBias(bias)
spline_z.SetDefaultContinuity(continuity)

# Evaluate spline and create polyline.
number_of_output_points = 300
offset = 1.0
...
profile_actor.GetProperty().SetDiffuseColor(1, 1, 0.7)
...
render_window.SetWindowName("k spline")
