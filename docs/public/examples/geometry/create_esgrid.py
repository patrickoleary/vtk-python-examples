...
num_i, num_j, num_k = 5, 6, 7
spacing_i, spacing_j, spacing_k = 20, 10, 1

# Points: generate grid vertices
points = vtkPoints()
for z in range(0, num_k * spacing_k, spacing_k):
    for y in range(0, num_j * spacing_j, spacing_j):
        for x in range(0, num_i * spacing_i, spacing_i):
...
for k in range(num_k - 1):
    for j in range(num_j - 1):
        for i in range(num_i - 1):
            ...
            cell_point_ids = np.ravel_multi_index(multi_index, (num_i, num_j, num_k), order='F')
            cells.InsertNextCell(8, cell_point_ids)

# Explicit structured grid: assemble the grid
explicit_grid = vtkExplicitStructuredGrid()
explicit_grid.SetDimensions(num_i, num_j, num_k)
explicit_grid.SetPoints(points)
explicit_grid.SetCells(cells)

# Convert: explicit structured grid → unstructured grid
to_unstructured = vtkExplicitStructuredGridToUnstructuredGrid()
to_unstructured.SetInputData(explicit_grid)
...
render_window.SetWindowName("create esgrid")
...
renderer.GetActiveCamera().SetPosition(8.383354, -72.468670, 94.262605)
renderer.GetActiveCamera().SetFocalPoint(42.295234, 21.111537, -0.863606)
renderer.GetActiveCamera().SetViewUp(0.152863, 0.676710, 0.720206)
renderer.GetActiveCamera().SetDistance(137.681759)
renderer.GetActiveCamera().SetClippingRange(78.173985, 211.583658)
