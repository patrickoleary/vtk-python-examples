...
# Compute cell gradients on an unstructured grid and visualize with tube edges and arrow glyphs.
...
# Convert point data to cell data, then compute cell gradients
pd2cd = vtkPointDataToCellData()
pd2cd.SetInputConnection(grid_reader.GetOutputPort())

gradients = vtkGradientFilter()
gradients.SetInputConnection(pd2cd.GetOutputPort())
gradients.SetInputScalars(vtkDataObject.FIELD_ASSOCIATION_CELLS, "scalars")

# Cell centers for glyph placement
cell_centers = vtkCellCenters()
cell_centers.SetInputConnection(gradients.GetOutputPort())

# Assign gradients as vectors
vectors = vtkAssignAttribute()
vectors.SetInputConnection(cell_centers.GetOutputPort())
...
render_window.SetWindowName("unstructuredgrid cell gradients")
